...
後台「店內配置」頁（biru-admin `/store-layout`，限平台管理員）可旋轉並切換顧客、工作人員、俯瞰視角。已定型號的設備（磨豆機、製冰機、手沖壺）用[購買清單](equipment.md)的實際尺寸，房間 9m × 7m × 2.8m 與其餘設備仍是假設值；量測或定型號後改 biru-admin 的 `constants/storeLayout.ts` 即可重算。
